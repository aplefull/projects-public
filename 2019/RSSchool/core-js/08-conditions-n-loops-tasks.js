...
function getFizzBuzz(num) {
  ...
  else if (str === '') return num.toString();
  return str;
}
...
function isCreditCardNumber(ccn) {
  ...
  let val = 0;
  const res = arr.reduce((acc, v) => { const vNum = +v; val += vNum; return val; }, 0).toString();
  // res = val.toString();
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes.length - 1; i += 1) {
    if (pathes[i][0] !== pathes[i + 1][0]) return '';
  }
...
function getMatrixProduct(a, b) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    matrix[i] = new Array(b[0].length);
  }

  for (let i = 0; i < a.length; i += 1) {
    for (let n = 0; n < b[0].length; n += 1) {
...
function evaluateTicTacToePosition(field) {
  for (let i = 0; i < 3; i += 1) {
    if (field[i][0] === field[i][1] && field[i][1] === field[i][2] && field[i][2] !== undefined) {
      return field[i][2];
    }
  }

  for (let i = 0; i < 3; i += 1) {
    if (field[0][i] === field[1][i] && field[1][i] === field[2][i] && field[2][i] !== undefined) {
      return field[2][i];
    }
  }

  if (field[0][0] === field[1][1] && field[1][1] === field[2][2] && field[2][2] !== undefined) {
    return field[2][2];
  }

  if (field[0][2] === field[1][1] && field[1][1] === field[2][0] && field[2][0] !== undefined) {
    return field[2][0];
  }
